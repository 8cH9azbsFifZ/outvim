...
if !exists("s:otl_loaded_functions")
  ...
  " Insert doable class
  function s:OtlSafelyInsertClass()
    if match(getline("."),"[\.\*]") == -1
      let @x = "* "
      normal! ^"xP
    endif
  ...
  " switch item to waiting
  function s:OtlSwitchClass()
    let questa = strridx(getline("."),"\.")
    let questb = strridx(getline("."),"\*")
  ...
  function s:OtlInd(line)
    return indent(a:line) / &tabstop
  endf

  " returns the line number of the root parent for any child
  function s:OtlFindRootParent(line)
    if s:OtlInd(a:line) == 0
      return (a:line)
    endif
    let l:i = a:line
    while l:i > 1 && s:OtlInd(l:i) > 0
      let l:i = l:i - 1
    endwhile
    return l:i
  endf
  ...
  function s:OtlNewHMD(line)
     let l:done = 0
     let l:count = 0
     let l:i = 1
     while s:OtlInd(a:line) < s:OtlInd(a:line+l:i)
        if (s:OtlInd(a:line)+1) == (s:OtlInd(a:line+l:i))
           let l:childdoneness = s:OtlNewHMD(a:line+l:i)
...
if !exists("b:otl_installed_mappings")
  ...
  if !no_otl_maps && (!exists("no_plugin_maps") || !no_plugin_maps)
    ...
    " Toggle item doable
    nnoremap <buffer><silent><script> <LocalLeader>c    :call <SID>OtlSafelyInsertClass()<CR>  :call<SID>OtlNewHMD(<SID>OtlFindRootParent(line(".")))<CR>
    nnoremap <buffer><silent><script> <LocalLeader>x    :call <SID>OtlSwitchClass()<CR>        :call<SID>OtlNewHMD(<SID>OtlFindRootParent(line(".")))<CR>
    nnoremap <buffer><silent><script> <LocalLeader>z    :call <SID>OtlNewHMD(<SID>OtlFindRootParent(line(".")))<CR>
    ...
    nnoremap <buffer><silent><script> _                 :call <SID>OtlOpenThisFold()<CR>
    nnoremap <buffer><silent><script> <LocalLeader>_    :call <SID>OtlOpenThisFold()<CR>
    ...
    " Tag jumps
    nnoremap <buffer><silent><script> <C-K>             :call <SID>OtlTagJump()<CR>
    nnoremap <buffer><silent><script> g<C-K>            :call <SID>OtlTagJump()<CR>
    nnoremap <buffer><silent><script> g<LeftMouse>      :call <SID>OtlTagJump()<CR>
    " And returns
    nnoremap <buffer><silent><script> <C-N>             :call <SID>OtlTagReturn()<CR>
